export default function parseOutline(outline: string) {
    let html = "";
    ...
    raw.forEach((line, i) => {
        ...
        if (id.length >= 2) return console.log(`Elements cannot have multiple IDs.`);

        if (name && tags.includes(name))
            html += `<${name}${
                classes.length ? ` class="${classes.map((c) => c.slice(1)).join(" ")}"` : ""
            }${id.length ? ` id="${id[0].slice(1)}"` : ""}>${line
